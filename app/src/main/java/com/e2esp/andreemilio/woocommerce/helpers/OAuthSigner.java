package com.e2esp.andreemilio.woocommerce.helpers;
...
public class OAuthSigner {
    ...
    public LinkedHashMap<String, String> getSignature(RequestMethod requestMethod, String endpoint, HashMap<String, String> params) {
        OAuthParameters parameters = new OAuthParameters();
        parameters.computeTimestamp();
        parameters.computeNonce();
        parameters.version = "1.0";
        ...
        try {
            ...
            genericUrl.appendRawPath(endpoint);
        //genericUrl.set("filter[category]", .getName());
        //genericUrl.set("filter[limit]", 5);
            OAuthHmacSigner oAuthHmacSigner = new OAuthHmacSigner();
            oAuthHmacSigner.clientSharedSecret = wooCommerce.getWc_secret();

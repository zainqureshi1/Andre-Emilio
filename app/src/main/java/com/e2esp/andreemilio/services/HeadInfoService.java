package com.e2esp.andreemilio.services;
...
/**
 * Initiated from CallerDetector Broadcast Receiver.
...
public class HeadInfoService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if(intent != null){
            ...
            String selectionOrder = DezynishContract.OrdersEntry.COLUMN_BILLING_PHONE+ " LIKE ? OR  " +
                    DezynishContract.OrdersEntry.COLUMN_CUSTOMER_BILLING_PHONE + " LIKE ?" ;
            String[] selectionOrderArgs = new String[]{ "%"+search+"%",
                    "%"+search+"%"};
            Cursor cursorOrder = getContentResolver().query(DezynishContract.OrdersEntry.CONTENT_URI,
                    projectionOrders,
                    selectionOrder,
                    selectionOrderArgs,
                    null);
            */
            ...
            String[] projectionCustomer = {
                    AndreEmilioContract.CustomerEntry.COLUMN_JSON,
            };
            String selectionCustomer = AndreEmilioContract.CustomerEntry.COLUMN_SHIPPING_PHONE+ " LIKE ? OR  " +
                    AndreEmilioContract.CustomerEntry.COLUMN_BILLING_PHONE + " LIKE ?" ;
            String[] selectionCustomerArgs = new String[]{ "%"+search+"%",
                    "%"+search+"%"};

            Cursor cursorCustomer = getContentResolver().query(AndreEmilioContract.CustomerEntry.CONTENT_URI,
                    projectionCustomer,
                    selectionCustomer,
                ...
                if(cursorCustomer.moveToFirst()) {
                    do {
                        ...
                        if(json!=null){
                            ...
                            String selectionOrderByCustomer = AndreEmilioContract.OrdersEntry.COLUMN_CUSTOMER_ID + " = ?" ;
                            String[] selectionOrderByCustomerArgs = new String[]{ String.valueOf(customer.getId()) };
                            Cursor cursorOrderByCustomer = getContentResolver().query(AndreEmilioContract.OrdersEntry.CONTENT_URI,
                                    projectionOrders,
